...
awaitG(() => {
    ...
    chrome.tabs.getCurrent(function (tabs) {
        ...
        if (!_requestId.length) {
            ...
            $("#downStream").prop("checked", false).prop("disabled", true);
...
function start() {
    ...
    const addHtml = (fragment) => {
        ...
        // 流式下载处理
        if (false && (_downStream || G.downStream) && !_ffmpeg) {
            fragment.fileStream = streamSaver.createWriteStream(fragment.downFileName).getWriter();
        }
    }
    ...
    down.on('completed', function (buffer, fragment) {
        ...
        chrome.storage.local.get(['herovideo_subfolder'], function(res) {
            ...
            const fullPath = subfolder ? (subfolder.endsWith('/') ? subfolder : subfolder + '/') + fragment.downFileName : fragment.downFileName;
