...
function init() {
    ...
    $("#StreamSaver").prop("checked", false).prop("disabled", true).attr("title", "Disabled in AI2Hero workspace mode");
...
$(".openDir").click(function () {
    ...
    chrome.storage.local.get(['herovideo_subfolder'], function(res) {
        ...
        if (workspaceSlug) {
            chrome.runtime.sendMessage({ Message: "ensureWorkspaceFolder", workspaceSlug, open: true });
            return;
        }
...
$("#StreamSaver").on("change", function () {
    ...
    $progress.html("<b>StreamSaver disabled in AI2Hero workspace mode.</b>");
});
...
$("#recorder").click(function () {
    if ($(this).data("switch") == "on") {
        ...
        alert("Live recorder requires StreamSaver and is disabled in AI2Hero workspace mode.");
        return;
        recorder = false;
...
$("#mergeTs").click(async function () {
    ...
    // 估算检查文件大小
    $("#StreamSaver").prop("checked", false);
    if (estimateFileSize > G.chromeLimitSize) {
        alert("StreamSaver is disabled in AI2Hero workspace mode. Large files will still use the configured workspace download path.");
    }
...
function apiDownload(fileBlob, fileName, ext) {
    chrome.storage.local.get(['herovideo_subfolder'], function(res) {
        ...
        const fullPath = subfolder ? (subfolder.endsWith('/') ? subfolder : subfolder + '/') + fullFileName : fullFileName;
        chrome.downloads.download({
