...
// ═══════════════════════════════════════════════════════
// HERO COCCOC — Tải video tự động qua Cốc Cốc (MVP)
// ═══════════════════════════════════════════════════════

export const coccocWorkers = pgTable('coccoc_workers', {
  id: serial('id').primaryKey(),
  teamId: integer('team_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
...
export const coccocProfiles = pgTable('coccoc_profiles', {
  id: serial('id').primaryKey(),
  teamId: integer('team_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
  userId: integer('user_id')
    ...
    .references(() => users.id, { onDelete: 'cascade' }),
  name: varchar('name', { length: 200 }).notNull(),
  userDataPath: text('user_data_path').notNull(),
  profileDir: varchar('profile_dir', { length: 100 }).notNull().default('Default'),
  status: varchar('status', { length: 20 }).notNull().default('active'), // 'active' | 'inactive'
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});

export const coccocProjects = pgTable('coccoc_projects', {
  id: serial('id').primaryKey(),
  teamId: integer('team_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
  ...
  profileId: integer('profile_id')
    .notNull()
    .references(() => coccocProfiles.id, { onDelete: 'cascade' }),
  name: varchar('name', { length: 200 }).notNull(),
  downloadFolder: text('download_folder').notNull(),
  schedule: varchar('schedule', { length: 20 }).notNull().default('manual'), // 'manual' | '60m' | '12h' | '24h'
  quality: varchar('quality', { length: 20 }).notNull().default('highest'), // 'highest' | '720p' | '480p'
  minDuration: integer('min_duration'), // seconds
  maxDuration: integer('max_duration'), // seconds
  priority: varchar('priority', { length: 20 }).notNull().default('newest'), // 'newest' | 'most_viewed'
  maxVideosPerRun: integer('max_videos_per_run').notNull().default(10),
  maxTotalVideos: integer('max_total_videos').notNull().default(100),
...
export const coccocSources = pgTable('coccoc_sources', {
  id: serial('id').primaryKey(),
  projectId: integer('project_id')
    .notNull()
    .references(() => coccocProjects.id, { onDelete: 'cascade' }),
  sourceType: varchar('source_type', { length: 30 }).notNull(), // 'channel_link' | 'search_keyword' | 'direct_link'
  sourceValue: text('source_value').notNull(),
  platform: varchar('platform', { length: 30 }), // 'tiktok' | 'douyin' | 'youtube' | 'facebook' | 'other'
  label: varchar('label', { length: 200 }),
  createdAt: timestamp('created_at').notNull().defaultNow(),
});

export const coccocTasks = pgTable('coccoc_tasks', {
  id: serial('id').primaryKey(),
  teamId: integer('team_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
  projectId: integer('project_id')
    .references(() => coccocProjects.id, { onDelete: 'set null' }),
  sourceId: integer('source_id')
    .references(() => coccocSources.id, { onDelete: 'set null' }),
  ...
  status: varchar('status', { length: 20 }).notNull().default('pending'), // 'pending' | 'scanning' | 'downloading' | 'completed' | 'failed' | 'skipped'
  error: text('error'),
  priority: integer('priority').notNull().default(0), // 0 = standard, 1 = quick download
  logs: jsonb('logs').default([]),
  workerId: integer('worker_id').references(() => coccocWorkers.id, { onDelete: 'set null' }),
  startedAt: timestamp('started_at'),
  completedAt: timestamp('completed_at'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
...
// Relations
export const coccocWorkersRelations = relations(coccocWorkers, ({ one, many }) => ({
  team: one(teams, {
    fields: [coccocWorkers.teamId],
    references: [teams.id],
  }),
  user: one(users, {
    fields: [coccocWorkers.userId],
    references: [users.id],
  }),
  tasks: many(coccocTasks),
}));

export const coccocProfilesRelations = relations(coccocProfiles, ({ one, many }) => ({
  team: one(teams, {
    fields: [coccocProfiles.teamId],
    references: [teams.id],
  }),
  user: one(users, {
    fields: [coccocProfiles.userId],
    references: [users.id],
  }),
  projects: many(coccocProjects),
}));

export const coccocProjectsRelations = relations(coccocProjects, ({ one, many }) => ({
  team: one(teams, {
    fields: [coccocProjects.teamId],
    references: [teams.id],
  }),
  user: one(users, {
    fields: [coccocProjects.userId],
    references: [users.id],
  }),
  profile: one(coccocProfiles, {
    fields: [coccocProjects.profileId],
    references: [coccocProfiles.id],
  }),
  sources: many(coccocSources),
  tasks: many(coccocTasks),
}));

export const coccocSourcesRelations = relations(coccocSources, ({ one }) => ({
  project: one(coccocProjects, {
    fields: [coccocSources.projectId],
    references: [coccocProjects.id],
  }),
}));

export const coccocTasksRelations = relations(coccocTasks, ({ one }) => ({
  team: one(teams, {
    fields: [coccocTasks.teamId],
    references: [teams.id],
  }),
  project: one(coccocProjects, {
    fields: [coccocTasks.projectId],
    references: [coccocProjects.id],
  }),
  source: one(coccocSources, {
    fields: [coccocTasks.sourceId],
    references: [coccocSources.id],
  }),
  worker: one(coccocWorkers, {
    fields: [coccocTasks.workerId],
    references: [coccocWorkers.id],
  }),
}));

// Types
export type CoccocWorker = typeof coccocWorkers.$inferSelect;
export type NewCoccocWorker = typeof coccocWorkers.$inferInsert;
export type CoccocProfile = typeof coccocProfiles.$inferSelect;
export type NewCoccocProfile = typeof coccocProfiles.$inferInsert;
export type CoccocProject = typeof coccocProjects.$inferSelect;
export type NewCoccocProject = typeof coccocProjects.$inferInsert;
export type CoccocSource = typeof coccocSources.$inferSelect;
export type NewCoccocSource = typeof coccocSources.$inferInsert;
export type CoccocTask = typeof coccocTasks.$inferSelect;
export type NewCoccocTask = typeof coccocTasks.$inferInsert;
