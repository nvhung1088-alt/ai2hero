...
export async function runGrok(
  ...
  if (!apiKey) {
    throw new Error('Thiếu xAI API Key.');
  }
  ...
  try {
    if (actionSlug === 'chat_completion') {
      const model = input.model || 'grok-2-latest';
      let messages: any[] = [];
    ...
    if (error.name === 'AbortError') {
      throw new Error('Timeout: Phản hồi từ Grok quá chậm (quá 15s).');
    }
    throw new Error(`Lỗi Grok (xAI): ${error.message}`);
  }
}
