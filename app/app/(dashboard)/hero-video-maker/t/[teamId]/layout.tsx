...
import HeroVideoMakerSidebar from '../../hero-video-maker-sidebar';
...
export default async function HeroVideoMakerDynamicLayout({
  children,
  params
}: {
  ...
  const isPreview = await isPreviewMode('hero-video-maker', teamId);
  if (!activatedApps.includes('hero-video-maker') && !isPreview) {
    redirect('/dashboard');
  }

  return (
    <>
      {isPreview && <PreviewBanner appId="hero-video-maker" />}
      <div className="flex flex-col min-h-screen bg-gray-950 text-white w-full">
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0 relative">
          {/* Sidebar dọc riêng biệt của HeroVideoMaker */}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl space-y-3">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-violet-500 to-fuchsia-500 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-violet-500/10 shrink-0">
              ...
              {/* Menu điều hướng dọc chính của HeroVideoMaker */}
              <HeroVideoMakerSidebar teamId={team.id} />
            </div>
