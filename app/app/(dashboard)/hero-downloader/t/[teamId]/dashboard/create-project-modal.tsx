...
import { X, Save, Plus, Trash2, FolderOpen, Chrome } from 'lucide-react';
...
import { createDownloaderProjectAction } from '@/lib/db/hero-downloader-actions';
import { showToast } from '@/app/(dashboard)/sim/sim-ui-helpers';

export function CreateProjectModal({ isOpen, onClose, teamId, cookies = [], onProjectCreated }: CreateProjectModalProps) {
  ...
  const handleCreate = async () => {
    ...
    setIsSaving(true);
    // Create the project (using the first source as sourceUrl for now as MVP)
    const firstSource = sources.filter(s => s.value)[0];
    const res = await createDownloaderProjectAction({
      teamId,
      userId: 1, // Using 1 as a fallback for now, ideally pass from parent
      name: projectName,
    ...
    if (res.success && res.project) {
      ...
      showToast('Lỗi khi tạo dự án: ' + res.error, 'error');
    }
    setIsSaving(false);
  };
