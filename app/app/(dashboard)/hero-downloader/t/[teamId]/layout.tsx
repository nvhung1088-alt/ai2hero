...
export default async function HeroDownloaderDynamicLayout({
  children,
  params
}: {
  ...
  const isPreview = await isPreviewMode('hero-downloader', teamId);
  if (!activatedApps.includes('hero-downloader') && !isPreview) {
    redirect('/dashboard');
  }

  return (
    <>
      {isPreview && <PreviewBanner appId="hero-downloader" />}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl space-y-3">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-teal-500 to-cyan-500 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-cyan-500/10 shrink-0">
              ...
              <nav className="space-y-1">
                <Link
                  href={`/hero-downloader/t/${team.id}/dashboard`}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-teal-500/10 text-teal-400 font-medium transition-colors"
                >
                  <LayoutDashboard className="h-4 w-4" />
                  <span className="text-sm">Dự án Quét Tải</span>
                </Link>
                <Link
                  href={`/hero-downloader/t/${team.id}/settings`}
