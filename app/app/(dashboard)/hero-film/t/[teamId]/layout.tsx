...
import HeroFilmSidebarMenu from '../../hero-film-sidebar-menu';
...
export default async function HeroFilmDynamicLayout({
  children,
  params
}: {
  ...
  const isPreview = await isPreviewMode('hero-film', teamId);
  if (!activatedApps.includes('hero-film') && !isPreview) {
    redirect('/dashboard');
  }

  return (
    <>
      {isPreview && <PreviewBanner appId="hero-film" />}
      <div className="flex flex-col min-h-screen bg-gray-950 text-white w-full">
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0 relative">
          {/* Sidebar dọc riêng biệt của Hero Film */}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl space-y-3">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-rose-500 to-red-500 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-rose-500/10 shrink-0">
              ...
              {/* Menu điều hướng dọc chính của Hero Film */}
              <HeroFilmSidebarMenu teamId={team.id} />
            </div>
