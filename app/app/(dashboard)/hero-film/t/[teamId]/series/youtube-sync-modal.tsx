...
import { Youtube, X, Loader2, CheckCircle2, AlertCircle, Settings2, Sparkles, Play, Save, Share2, PauseCircle, PlayCircle, ChevronRight } from 'lucide-react';
...
export function YoutubeSyncModal({ 
  ...
  // AI Translation monitoring state
  const isPausedRef = useRef(false);
  const [isPaused, setIsPaused] = useState(false);
  const [isTranslating, setIsTranslating] = useState(false);
  const [translateChannelId, setTranslateChannelId] = useState<number | null>(null);
  const [currentTranslating, setCurrentTranslating] = useState<string>('');
        ...
        {activeTab === 'manage' && !results && !isSyncing && (
           <div className="space-y-3 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
             {channels.length === 0 ? (
               ...
               channels.map(c => (
                 <div key={c.id} className="p-4 bg-white/5 border border-white/10 hover:border-white/20 transition rounded-xl flex flex-col gap-3">
                   <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 bg-black/40 p-1.5 rounded-lg border border-white/5">
                      ...
                      {!(isTranslating && translateChannelId === c.id) && (
                        <button 
                          onClick={async () => {
                             if(isSyncing || isTranslating) return;
                             setIsTranslating(true);
                             setTranslateChannelId(c.id);
                             ...
                             try {
                               while (keepGoing && !isPausedRef.current) {
                                 ...
                                 if (countAdded > 0) {
                                   ...
                                   setTotalTranslated(currentTotal);
                                   setRemainingCount(res.remaining || 0);
                                   if (res.translatedTitles) {
                                      setRecentlyDone(prev => [...res.translatedTitles, ...prev].slice(0, 10)); // Giữ 10 cái gần nhất
                                   }
                                   
                                   await fetchChannels();
                                   if (res.remaining === 0) {
                                     keepGoing = false;
                                     setStatusText(`🎉 ĐÃ DỊCH HOÀN TẤT 100%! Đã biên dịch toàn bộ ${currentTotal} video trong dự án.`);
                                   }
                                 } else {
                                   keepGoing = false;
                                   if (currentTotal === 0) {
                                     ...
                                     setStatusText(`🎉 Hoàn tất! Tổng cộng đã biên dịch ${currentTotal} video.`);
                                   }
                                 }
                                 // Thoát sớm nếu user pause
                                 if (isPausedRef.current) break;
                               }
                               if (!isPausedRef.current) {
                                 setTimeout(() => {
                                   setIsTranslating(false);
                                   setTranslateChannelId(null);
                                 }, 3000);
                               }
                             } catch (e: any) { 
                                 setError(e.message || 'Lỗi không xác định'); 
                   ...
                   {/* Translate UI Panel */}
                   {isTranslating && translateChannelId === c.id && (
                      <div className="flex flex-col gap-2 bg-indigo-900/20 border border-indigo-500/20 rounded-lg p-3 w-full animate-in slide-in-from-top-2 duration-300">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <button 
                            onClick={() => {
                              // Nếu đang chạy thì bấm Tạm dừng
                              // Nếu đang tạm dừng thì bấm Tiếp tục -> phải gọi lại hàm click
                              if (!isPaused) {
                                ...
                                isPausedRef.current = false;
                                setIsPaused(false);
                                // Kích hoạt lại vòng lặp bằng cách click vào id ẩn hoặc gọi hàm
                                // Nhưng dễ nhất là click lại vào nút Dịch (nhưng nó đang bị hide).
                                // Ta sẽ tự implement hàm resume:
                                const resumeTranslation = async () => {
                                   ...
                                   try {
                                     while (keepGoing && !isPausedRef.current) {
                                       ...
                                       const countAdded = res.count || 0;
                                       if (countAdded > 0) {
                                         setTotalTranslated(prev => prev + countAdded);
                                         setRemainingCount(res.remaining || 0);
                                         if (res.translatedTitles) {
                                            setRecentlyDone(prev => [...res.translatedTitles, ...prev].slice(0, 10));
                                         }
                                         await fetchChannels();
                                     ...
                                     if (!isPausedRef.current && remainingCount === 0) {
                                       setTimeout(() => { setIsTranslating(false); setTranslateChannelId(null); }, 3000);
                                     }
                                   } catch (e: any) { setError(e.message); setIsTranslating(false); setTranslateChannelId(null); }
                                };
                                resumeTranslation();
                              }
                            }}
                            className="px-2 py-1 bg-indigo-500/20 hover:bg-indigo-500/30 rounded text-[10px] font-bold text-indigo-300 flex items-center gap-1 transition"
                          >
                        ...
                        <div className="w-full bg-black/40 rounded-full h-2 overflow-hidden border border-white/5 relative">
                           <div 
                             ...
                             style={{ width: `${Math.max(2, (totalTranslated / (totalTranslated + remainingCount || 1)) * 100)}%` }}
                           >
                             {!isPaused && <div className="absolute top-0 right-0 bottom-0 left-0 bg-white/20 animate-pulse"></div>}
                           </div>
                        </div>
                        <div className="flex justify-between text-[10px] text-gray-400">
                          <span>Đã dịch: <strong className="text-white">{totalTranslated}</strong>/{totalTranslated + remainingCount} video</span>
                          <span className="flex items-center gap-1">
                            {!isPaused && <span className="relative flex h-2 w-2 mr-0.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span></span>}
                            {isPaused ? 'Đang tạm dừng' : 'Đang chạy ngầm...'}
                          </span>
                        </div>

                        {/* Recent translations */}
                        {recentlyDone.length > 0 && (
                          <div className="mt-1 bg-black/40 rounded p-2 border border-white/5 max-h-36 overflow-y-auto custom-scrollbar flex flex-col gap-1.5">
                            <div className="text-[9px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">Vừa hoàn thành:</div>
                            {recentlyDone.map((item, idx) => (
                              <div key={idx} className="flex gap-2 items-start bg-white/5 rounded px-2 py-1.5">
                                <CheckCircle2 className="w-3.5 h-3.5 text-green-500 mt-0.5 shrink-0" />
                                <div className="text-[10px] leading-snug flex-1 min-w-0">
                                  <span className="text-gray-200 font-medium block truncate">{item.seriesTitle}</span>
                                  <span className="text-gray-400 italic truncate w-full block">{item.summary}</span>
                                </div>
                              </div>
                            ))}
                          </div>
