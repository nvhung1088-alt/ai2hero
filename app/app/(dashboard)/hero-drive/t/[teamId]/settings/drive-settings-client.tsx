    ...
    <div className="flex-1 p-6 space-y-6 bg-slate-950/60 min-h-screen text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-800 gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            ...
            Cài Đặt & Quản Lý Tài Khoản Google Drive
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Quản lý các tài khoản Google Drive đã liên kết để phân bổ cho từng dự án quét upload.
          </p>
        </div>
        ...
        >
          ...
          Thêm tài khoản Drive mới qua Connect Hub
        </Link>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">
          Tài khoản Google Drive hiện có ({googleDriveConnections.length})
        </h2>
        ...
        {googleDriveConnections.length === 0 ? (
          <div className="p-8 border border-slate-800 rounded-xl bg-slate-900/30 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1 mb-4">
              Vui lòng kết nối ít nhất 1 tài khoản Google Drive trong Connect Hub để bắt đầu phân bổ thư mục quét.
            </p>
            <Link
              href={`/connect-hub/t/${teamId}/dashboard`}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h3 className="font-semibold text-sm text-slate-200">{conn.name}</h3>
                      <p className="text-[11px] text-slate-400">ID Kết nối: #{conn.id}</p>
                    </div>
                  </div>
                ...
                <div className="text-xs space-y-1 text-slate-400 pt-2 border-t border-slate-800/80">
                  <p className="flex items-center gap-1 text-[11px]">
                    ...
                    Xác thực: OAuth 2.0 Refresh Token
                  </p>
                </div>
      ...
      <div className="p-5 bg-slate-900/40 border border-slate-800 rounded-xl space-y-3 text-xs">
        ...
        <p className="text-slate-300">
          Khi bạn tạo nhiều Dự án và phân bổ nhiều Thư mục máy tính (Multi-folder mappings), bạn có thể chạy Python Worker theo lệnh:
        </p>
