...
import HeroDubSidebarMenu from '../../hero-dub-sidebar-menu';
...
export default async function HeroDubDynamicLayout({
  children,
  params
}: {
  ...
  const isPreview = await isPreviewMode('hero-dub', teamId);
  if (!activatedApps.includes('hero-dub') && !isPreview) {
    redirect('/dashboard');
  }

  return (
    <>
      {isPreview && <PreviewBanner appId="hero-dub" />}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl space-y-3">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-orange-500/10 shrink-0">
              ...
              {/* Sidebar Menu */}
              <HeroDubSidebarMenu teamId={team.id} />
            </div>
