if (!window.hasAi2HeroBridgeGemini) {
  ...
  // Kiểm tra Đăng nhập: Dựa vào sự TỒN TẠI của khung chat thay vì dò nút đăng nhập
  function checkAuthStatus() {
    // Nếu URL là trang đăng nhập -> chưa đăng nhập
    if (window.location.href.includes('/signin') || window.location.hostname.includes('accounts.google.com')) {
      return true; // Cần đăng nhập
    }
    // Nếu không tìm thấy khung chat nào -> chưa đăng nhập (trang loading hoặc trang login)
    ...
    return !hasChatInput; // Không có input = cần đăng nhập
  }
  ...
  async function processGeminiJob(promptText, attachments) {
    ...
    if (!inputEl) {
      throw new Error('Không tìm thấy khung nhập liệu trên giao diện Gemini. Hãy đăng nhập và thử lại.');
    }
    ...
    // 6. Chờ phản hồi v2
    return await waitForGeminiResponse(promptText, baselineText);
  }
  ...
  function waitForGeminiResponse(promptText = '', baselineText = '') {
    return new Promise((resolve, reject) => {
      ...
      const quietWait = isImageGen ? 15000 : 3000; // Đợi ngắt nhảy chữ: 15s cho ảnh, 3s cho dịch text
      ...
      const globalTimeout = setTimeout(() => {
        ...
        if (text && text !== baselineText) {
          ...
          reject(new Error('Timeout quá 4 phút không nhận được câu trả lời từ Gemini Web.'));
        }
      }, MAX_WAIT_MS);
      ...
      const observer = new MutationObserver(() => {
        ...
        // Chỉ tính là có kết quả mới khi text khác baseline cũ
        if (currentText && currentText !== baselineText) {
          
          // Phát hiện AI sinh chữ xong thông qua trạng thái nút Stop Generating biến mất / nút Send sáng lại
          const stopBtn = document.querySelector('button[aria-label*="Stop"], button[aria-label*="Dừng"], button:has(svg rect)');
          const sendBtn = document.querySelector('button[aria-label*="Send"]');
          
          if (!stopBtn && sendBtn && !sendBtn.disabled && currentText !== lastText) {
             console.log('[Ai2Hero Bridge] Phát hiện nút Stop biến mất và nút Send đã kích hoạt trở lại. Hoàn tất!');
             observer.disconnect();
          ...
          if (currentText !== lastText) {
            ...
            // Fallback: Nếu không phát hiện được nút, tự động resolve khi văn bản ngừng thay đổi trong khoảng quietWait
