...
- **Tiến độ tích hợp:**
  ...
  - `[x]` 🔄 **Khắc phục Triệt Để Lỗi Cứu Hộ Tầng 2 DeepSeek Cloud API & Tối Ưu Hóa Kết Nối Server**: Xác định nguyên nhân gốc do domain `www.ai2hero.com` bị Vercel trả về `402 Payment Required (DEPLOYMENT_DISABLED)`. Chuẩn hóa mặc định `API_BASE_URL` về `https://ai2hero-flax.vercel.app/api/hero-dub` trên cả `herodub_worker.py` và file phát hành `herodub_worker.py?v=24`. Bổ sung cơ chế Auto-Failover tự động chuyển domain nếu gặp 402/lỗi mạng. Tinh gọn sub-batch size thành 25 câu, tăng timeout client 75s (DeepSeek hoàn thành trong 4-7s). Khắc phục cơ chế nuốt lỗi để log rõ HTTP status và message lỗi thực tế. Đồng thời nâng cấp route `/api/hero-dub/translate` bảo toàn kết quả đã dịch tốt của DeepSeek sang Quality Gate và hỗ trợ linh hoạt trạng thái kết nối (`connected` | `active`).
