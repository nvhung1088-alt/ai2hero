...
- **Tiến độ tích hợp:**
  ...
  - `[x]` 📝 **Nâng Cấp Publishing Suite & Hoàn Thiện Video Không Lời (Silent / Music Video Pipeline)**: Khắc phục triệt để lỗi ném exception làm hỏng task khi video không có track âm thanh hoặc Whisper STT trả về 0 câu thoại (Empty Speech). Worker tự động nhận diện và bypass STT & TTS an toàn, render video thành phẩm giữ nguyên âm thanh gốc, và tiến thẳng sang AI Publishing Suite để hoàn thiện video. Xóa bỏ hoàn toàn lỗi mô tả ngắn củn dính chữ Hán; xây dựng engine `build_rich_vietnamese_description` tạo mô tả 3 đoạn chuẩn SEO chuyên nghiệp (Hook mở màn, chi tiết công đoạn chế tác/sinh tồn/ẩm thực, cảm xúc ASMR thư giãn kèm CTA kêu gọi Like/Đăng ký) cam kết 100% Tiếng Việt sạch sẽ trên cả Server API (`/api/hero-dub/copywriting`), Worker cục bộ (`herodub_worker.py`) và công cụ sửa hàng loạt (`fix_publishing_suite.py`).
