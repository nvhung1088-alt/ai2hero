...
- **2026-07-30 (hero-downloader - Worker Safe Speed Optimization)**:
  - ⚡ **Strict 1-Stream Multi-platform Safety**: Cấu hình chuẩn `concurrent_fragment_downloads = 1` trong `downloader.py` theo yêu cầu an toàn tuyệt đối của User, đảm bảo 1 video chỉ tải qua đúng 1 luồng kết nối đơn thuần, phòng ngừa 100% rủi ro Bilibili/Douyin bóp băng thông hay hỏng file.
  - ⚡ **Optimized Sleep Delay**: Hạ `sleep_requests` từ `1.5s` xuống `0.3s` (nghỉ nhẹ 300ms giữa các HTTP request), loại bỏ hơn 80% thời gian hoãn vô ích khi tải danh sách video.
  - ⚡ **1MB RAM Buffer I/O**: Nâng `chunk_size` đệm đọc/ghi đĩa trong `download_direct_mp4()` từ `64KB` lên `1MB` (`1048576 bytes`), giảm 94% syscall I/O, giúp CPU nhẹ hơn và dữ liệu truyền mượt mà hơn.
  - 🚀 **Dynamic Video Reordering & Vercel Deploy**: Cấu hình thuật toán SQL `CASE WHEN` (`hero-downloader-actions.ts`) kết hợp `getStatusRank` Client Side (`downloader-dashboard-client.tsx`) tự động sắp xếp danh sách video theo 3 nhóm ưu tiên: **Đang tải (Xếp ĐẦU)** ➔ **Chờ tải / Paused (Xếp GIỮA)** ➔ **Hoàn tất / Thất bại (Xếp DƯỚI CÙNG)** (`commit b562c44`).
