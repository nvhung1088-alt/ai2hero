...
- **Tiến độ tích hợp:**
  ...
  - `[x]` [Phase 5] Tách biệt quy trình 2 bước: Kéo video siêu tốc ➔ AI Biên dịch theo yêu cầu. Bổ sung hiển thị `Đã kéo: X video • Đã dịch AI: Y video` chính xác theo từng kênh và nút 1-click **"✨ Dịch Tất Cả Video"** kích hoạt vòng lặp dịch cuốn chiếu toàn bộ video chưa dịch trong dự án.
  - `[x]` [Phase 6] Khắc phục lỗi SQL PostgreSQL (`jsonb_array_length`), cho phép dịch ngầm không vách ngăn (Background Processing) và tích hợp Widget hiển thị tiến độ sống động `⏳ Dịch AI Dự Án (Processed/Total)` ngay góc trái thanh điều hướng Sidebar.
...
- **Tiến độ tích hợp:**
  ...
  - `[x]` 🔄 **Khắc phục Triệt Để Lỗi Cứu Hộ Tầng 2 DeepSeek Cloud API & Tối Ưu Hóa Kết Nối Server**: Xác định nguyên nhân gốc do domain `www.ai2hero.com` bị Vercel trả về `402 Payment Required (DEPLOYMENT_DISABLED)`. Chuẩn hóa mặc định `API_BASE_URL` về `https://ai2hero-flax.vercel.app/api/hero-dub` trên cả `herodub_worker.py` và file phát hành `herodub_worker.py?v=24`. Bổ sung cơ chế Auto-Failover tự động chuyển domain nếu gặp 402/lỗi mạng. Tinh gọn sub-batch size thành 25 câu, tăng timeout client 75s (DeepSeek hoàn thành trong 4-7s). Nâng cấp tính chịu lỗi tại `/api/hero-dub/translate`: Nới lỏng Mini-Rescue cho mọi kích thước batch (kể cả batch ngắn 12 câu), loại bỏ HTTP 500 khi DeepSeek trả về thiếu nhằm bảo toàn tối đa các câu đã dịch tốt chuyển sang Worker Quality Gate dọn dẹp, hỗ trợ linh hoạt trạng thái kết nối (`connected` | `active`). Nâng cấp Worker logging bóc tách trực tiếp mã `error` và `detail` từ JSON phản hồi khi HTTP khác 200.
  - `[x]` 🎯 **Nâng Cấp Multi-Genre Adaptive Copywriting & Xử Lý Video Không Lời (Silent / Music Video Pipeline)**: Khắc phục triệt để lỗi ném exception làm hỏng task khi video không có track âm thanh hoặc Whisper STT trả về 0 câu thoại (Empty Speech). Worker tự động nhận diện và bypass STT & TTS an toàn, render video thành phẩm giữ nguyên âm thanh gốc, và tiến thẳng sang AI Publishing Suite để hoàn thiện video. Xóa bỏ hoàn toàn việc gán cứng nội dung "sinh tồn hoang dã"; xây dựng bộ nhận diện thể loại thông minh `detect_video_genre()` và engine sinh mô tả 3 đoạn chuẩn SEO thích ứng theo 6 nhóm thể loại lớn: Hoạt hình 3D (Anime/Donghua/Tiên hiệp), Phim ngắn (Drama/Đô thị/Review), Ẩm thực (Nấu ăn/Food/ASMR Cooking), Sinh tồn (Bushcraft/Chế tác), Khoa học (Khám phá/Tài liệu), và Đời sống (Vlog/Thư giãn). Tự động sinh hashtags và văn phong chuẩn xác, cam kết 100% Tiếng Việt sạch sẽ trên cả Server API (`/api/hero-dub/copywriting`), Worker cục bộ (`herodub_worker.py`), bản phát hành tải về (`app/public/uploads/herodub_worker.py`) và công cụ sửa hàng loạt (`fix_publishing_suite.py`).
  - `[x]` 👁️ **Tích Hợp Multimodal Vision Copywriting & Chuẩn Hóa Redesign Thumbnail Không Khung Đen**:
    - **Multimodal Copywriting Engine**: Gửi trực tiếp ảnh bìa thumbnail gốc (`thumb_src` dạng Base64) kèm tên file gốc và các câu thoại phụ đề sang Gemini Web qua Browser Bridge. AI nhìn thấy nhân vật, bối cảnh (ví dụ: chú heo bông, bác sĩ, phim X-quang) và đọc được dòng chữ Trung Quốc trên bìa để sáng tạo tiêu đề chuẩn xác, hấp dẫn, đúng thể loại.
    - **Bộ Lọc Tiêu Đề Thông Minh `is_meaningful_title()`**: Khắc phục triệt để lỗi tiêu đề bị rác/vô nghĩa (như `2492_Vlog_AI____` do việc xóa chữ Hán để lại các ký tự tiếng Anh/số rời rạc). Tự động kiểm tra độ dài chữ cái thực tế và số lượng từ, kích hoạt cơ chế dịch cứu hộ 3 tầng từ tên gốc tiếng Trung hoặc câu thoại tiếng Việt tiêu biểu nhất.
    - **Chuẩn Hóa Pipeline Thiết Kế Ảnh Bìa 2 Bước Tuần Tự (Gemini Inpainting + Worker Typography)**:
      + **Bước 1 (Gemini Inpainting / Text Removal)**: Gemini (Browser AI Bridge qua Gemini Web Free 0đ hoặc Flash Image API) chỉ làm duy nhất 1 nhiệm vụ: XÓA SẠCH 100% chữ tiếng Trung Quốc và watermark trên ảnh, bảo tồn nguyên vẹn 100% nhân vật, nét mặt, trang phục, ánh sáng và bối cảnh gốc, xuất ra ảnh nền sạch không chữ (`_clean_bg_...jpg`). Tuyệt đối không bắt Gemini tự vẽ chữ tiếng Việt (tránh lỗi sai dấu, méo chữ, dị dạng).
      + **Bước 2 (Worker Local Typography Engine)**: Python Worker nạp ảnh nền sạch, tự động nhận diện tỷ lệ khung hình (16:9 ngang dùng Font thư pháp/Arial Bold + Glow hào quang + 3D Shadow tại 65% chiều cao; 9:16 dọc dùng Gold 3D kép 7 lớp shadow tại 60-72% chiều cao ảnh) để viết tiêu đề tiếng Việt chuẩn 100% có dấu, viền đen tương phản cao vào **đúng vị trí chữ cũ**. Đã xóa bỏ triệt để tấm biển đen `BOTTOM PLATE` (từng che 32% ảnh), cam kết ảnh bìa thoáng mắt và chủ thể nổi bật. Đồng bộ hoàn toàn trên `herodub_worker.py`, bản phát hành tải về `app/public/uploads/herodub_worker.py` và công cụ sửa hàng loạt `fix_publishing_suite.py`.
...
- **2026-07-15 (Gỡ bỏ HeroSocial MVP & Postiz Integration)**:
  ...
  - 🚀 **Dọn dẹp Middleware & Config**: Gỡ toàn bộ rewrite proxy (định tuyến cổng 4200) và block SSO Sync ngầm sang Postiz Backend (port 5000) trong Next.js middleware.
  - 🚀 **Docker & Source clean up**: Dừng và xóa 4 containers postiz-postgres, postiz-redis, pg-admin và thực hiện xóa hoàn toàn thư mục apps/hero-postiz ngầm.
  - 🚀 **Verify**: TS compilation pass 100% không phát sinh lỗi.

- **2026-07-13 (hero-social - Bug Fixes & SSO/UI Enhancements)**:
  - 🚀 **Khắc phục lỗi 404 Next.js (Postiz Frontend)**: Đã phát hiện và sửa lỗi toàn bộ các route của Postiz (bao gồm `/analytics`, `/launches`, `/auth`) trả về 404 Not Found. Nguyên nhân do Turbopack bị kẹt cache `.next` thư mục sau khi cấu hình `basePath` và `.env` thay đổi. Giải pháp: Xóa cache và khởi động lại dev server, giúp AI2Hero proxy routing hoạt động hoàn hảo.
  - 🚀 **Thiết kế UI "Native App" mượt mà**: Ẩn hoàn toàn thanh Sidebar và Navbar của AI2Hero Core trong module `hero-social/page.tsx` để dành 100% không gian hiển thị cho iframe. Đồng thời nhúng iframe với tham số `?embed=true` để tắt Navbar/Sidebar nội bộ của Postiz. Kết quả: Postiz MVP hiển thị như một màn hình Native của AI2Hero không có bất kỳ menu dư thừa nào.
  - 🚀 **Khắc phục lỗi hiển thị Icon 404**: Cấu hình rewrite proxy `/icons/:path*`, `/peoplemarketplace.svg` và `/no-channels.svg` tại `next.config.ts` của AI2Hero, chuyển tiếp an toàn các tệp tin SVG bị mã hóa cứng (hardcoded paths) từ Postiz Frontend sang server 4200.
  - 🚀 **Tích hợp SSO Auto-Login**: Xây dựng cơ chế Single Sign-On tự động tại middleware của AI2Hero Core. Khi user truy cập `/hero-social` hoặc `/auth`, middleware tự động lấy email của user hiện tại từ session cookie, gọi API đăng ký/đăng nhập sang Postiz Backend ngầm ở Server-side, nhận JWT session token và Organization ID và tự set cookie `auth` & `showorg` cho client. Giúp người dùng dùng chung 1 tài khoản AI2Hero và tự động đăng nhập mượt mà vào HeroSocial (tránh tình trạng phải đăng nhập 2 lần).
  - 🚀 **Vá lỗi CORS & Chuyển hướng 404**: Sửa file cấu hình khởi tạo CORS của Postiz backend để cho phép cả hai origin local `http://localhost:3000` và `http://127.0.0.1:3000`. Đồng thời viết rewrite rules cho `/auth` và `/auth/:path*` trong `next.config.ts` để redirect trang login của Postiz không bị lỗi 404. Lỗi 401 Unauthorized ở endpoint `/copilot/chat` được xác định là do thiếu `OPENAI_API_KEY` chứ không phải do lỗi Proxy xác thực.
  - 🚀 **Khắc phục lỗi X-Frame-Options (Từ chối kết nối FB/Google)**: Nút "Thêm Kênh" của Postiz Frontend sử dụng `window.location.href`, dẫn đến việc load OAuth Login Screen của FB/Google ngay trong Iframe của HeroSocial. Do chính sách bảo mật X-Frame-Options của các mạng xã hội cấm nhúng Iframe, việc này đã gây ra lỗi "từ chối kết nối". Đã thay đổi thành `window.open` để xử lý xác thực trên Cửa sổ (Tab) mới, vượt tường lửa Iframe thành công.
  - 🚀 **Xây dựng UI Settings Cấu hình `.env` tự động (HeroSocial)**: Bổ sung màn hình cài đặt Admin tại `/settings/hero-social` với giao diện UI ShadCN chuyên nghiệp cho phép nhập Client ID/Secret của FB, YouTube, TikTok, Pinterest, LinkedIn và Twitter (X). Tích hợp với API Server Action Node.js đọc và cập nhật đè trực tiếp các tham số OAuth vào file `apps/hero-postiz/.env` bằng Regex, bỏ qua thao tác cấu hình thủ công cho Dev/Admin. Màn hình đã được thêm vào luồng UI_MAP.md.
  - 🚀 **Tích hợp sâu API Credentials vào Postiz Settings**: Nhằm tạo trải nghiệm đồng bộ 100%, tôi đã viết lại module Settings của Postiz Frontend (`settings.component.tsx`), bổ sung thêm tab "Mạng xã hội API". Tab này nhúng trực tiếp giao diện Cài đặt API thông qua một trang không layout cô lập `/settings-embed/hero-social`, giúp người dùng cấu hình khóa API ngay tại pop-up Settings bản địa của Postiz mà không phải nhảy trang ra ngoài.
  - 🚀 **Đồng bộ hóa giao diện Iframe tối màu (Dark Theme Fix)**: Ép class `dark` của Tailwind CSS vào container ngoài cùng của `/settings-embed/hero-social` để khắc phục lỗi các component ShadCN của AI2Hero Core bị xám trắng loang lổ khi tải trong iframe. Đồng thời bổ sung chi tiết **Hướng dẫn đăng ký ứng dụng** và link copy nhanh Redirect URI tương ứng cho từng nền tảng mạng xã hội ngay trên đầu form nhập.
  - 🚀 **Tinh chỉnh Sidebar Navigation của Postiz**: Xóa bỏ hoàn toàn logo có chữ "P" (Postiz logo) ở trên cùng sidebar của Postiz Frontend (`layout.component.tsx`) và di chuyển nút "Cài đặt" (Settings) từ menu dưới lên nằm trực tiếp bên dưới chữ "Tích hợp" (Integrations) trong menu trên (`top.menu.tsx`) theo mong muốn của khách hàng.

- **2026-07-12 (hero-social - Tích hợp Postiz App thành lõi HeroSocial)**:
  - 🚀 **Di chuyển & Sáp nhập Monorepo**: Clone 100% mã nguồn sạch của Postiz vào thư mục con [apps/hero-postiz](file:///C:/Users/ADMIN/OneDrive/Desktop/Ai2Hero/apps/hero-postiz). Cấu hình dependencies PNPM, build Prisma Client và chạy docker-compose local độc lập.
  - 🚀 **Nâng giới hạn RAM & Khắc phục OOM**: Cấu hình `NODE_OPTIONS="--max-old-space-size=8192"` trong `.env` để cấp 8GB RAM cho dev server, giải quyết triệt để lỗi crash JavaScript Heap Out Of Memory khi compile frontend.
  - 🚀 **Đăng ký Connector API Hub**: Khởi tạo definition `postiz.ts` và runner `postiz.ts` trong Connect Hub, đăng ký kết nối API (cổng 5000) hỗ trợ các hành động lấy accounts, tạo posts, xem analytics.
  - 🚀 **Nhúng giao diện (UI Integration)**: Viết lại trang dashboard của MVP HeroSocial tại [dashboard/page.tsx](file:///C:/Users/ADMIN/OneDrive/Desktop/Ai2Hero/app/app/%28dashboard%29/hero-social/t/%5BteamId%5D/dashboard/page.tsx) để nhúng trực tiếp giao diện Postiz Frontend qua Iframe Obsidian Glass, đạt chuẩn 0 errors compile TypeScript.
